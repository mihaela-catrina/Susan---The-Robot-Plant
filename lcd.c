...
void LCD_putCharAt(uint8_t line, uint8_t addr, char c) {
    /* TODO task 1 LCD */
    LCD_writeInstr(LCD_INSTR_clearDisplay);

    if (line == 1)
        LCD_writeInstr(LCD_INSTR_DDRAM | LCD_INSTR_nextLine);
    else
        LCD_writeInstr(LCD_INSTR_returnHome);

    for (int i = 1; i <= addr; i++)
        LCD_writeInstr(LCD_INSTR_cursorNext);
...
void LCD_createChar(uint8_t ch[]) {
    LCD_writeInstr(LCD_INSTR_CGRAM | (0 << 3));
